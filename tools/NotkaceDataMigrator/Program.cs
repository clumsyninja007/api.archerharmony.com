...
// One-time snapshot copy of the slim datasets from the source MySQL/MariaDB databases into
// Azure SQL. Only the columns each API needs are copied; table and column names are identical
// on both sides, so every copy is a straight column-subset transfer.
//
// Each "job" has its own Source (MySQL) and Target (Azure SQL) connection string, configured
// under Jobs:<name> in appsettings.json / user-secrets / environment (copy the example file).
// Run all jobs with no args, or name specific ones:  dotnet run -- TelegramBot
...
// Tables are ordered parent -> child so FK targets exist before the rows that reference them.
var jobs = new Dictionary<string, IReadOnlyList<TableSpec>>(StringComparer.OrdinalIgnoreCase)
{
    ["Notkace"] =
    [
    ...
    [
        new("chat_tracker",
        [
            Long("chat_id"), Str("first_name"), Str("last_name"),
            Bool("water_reminder"), Bool("active")
        ]),
    ],
};

var selected = args.Length > 0 ? args : jobs.Keys.ToArray();

foreach (var jobName in selected)
{
    if (!jobs.TryGetValue(jobName, out var tables))
    {
        Console.Error.WriteLine($"Unknown job '{jobName}'. Known jobs: {string.Join(", ", jobs.Keys)}");
        return 1;
    }

    var source = config[$"Jobs:{jobName}:Source"];
    var target = config[$"Jobs:{jobName}:Target"];
    if (string.IsNullOrWhiteSpace(source) || string.IsNullOrWhiteSpace(target))
    {
        Console.Error.WriteLine(
            $"Missing Jobs:{jobName}:Source / Jobs:{jobName}:Target. " +
            "Copy appsettings.example.json to appsettings.json and fill them in.");
        return 1;
    }

    await RunJob(jobName, source, target, tables);
}

Console.WriteLine("\nAll done.");
return 0;

static async Task RunJob(string name, string sourceConn, string targetConn, IReadOnlyList<TableSpec> tables)
{
    Console.WriteLine($"\n=== Job: {name} ===");

    await using var mysql = new MySqlConnection(sourceConn);
    await mysql.OpenAsync();
    Console.WriteLine($"Source MySQL: {mysql.Database}");

    await using var sql = new SqlConnection(targetConn);
    await sql.OpenAsync();
    Console.WriteLine($"Target Azure SQL: {sql.Database}\n");

    // Disable FK enforcement and clear existing rows (child -> parent) so the job is re-runnable.
    ...
    // make validation fail; report it and leave that table's constraints untrusted rather than
    // aborting the load.
